...
df = pd.read_csv('Output_Data.csv', index_col=0).to_numpy() #this loads the saved data from the simulation file and makes it a numpy array  
...
for i in range(0,int(len(df))):#this should loop should repeat for every value in the dataframe
    ...
    g+=1
    if g==int(len(df)+1):#This is so when the end of the row has been reached the code will select the start of the row
        n+=1
        g=0

plt.xlabel('Starting Theta1',fontsize=8)#These are the labels on the x and y axes to be plotted
plt.ylabel('Starting Theta2',fontsize=8)
plt.imshow(data, extent=[0,2*math.pi,0,2*math.pi], cmap=cm.magma, aspect='auto', interpolation='none')#This constructs the graph from the data nunmpy array 
